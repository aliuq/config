...
test_connect() {
  ...
  local limit=${2:-5}
  local now=$(date +%s)
  local cache_key="${url}_timestamp"
  local last_test_time=${cache[$cache_key]}

  # Check cache for recent result
  if [[ -n "$last_test_time" && $((now - last_test_time)) -lt $limit ]]; then
    return ${cache["${url}_result"]}
  fi
