...
if grep -iq "Microsoft" /proc/version; then
    ...
    echo -e " - Manually check Docker configuration on WSL. "
    echo "{
    ...
    if read_confirm "Manually check Docker configuration on docker desktop？(y/n): "; then
        echo " - Continuing with the installation."
    else
        echo " - Please check Docker configuration and restart Docker."
        exit 1
    fi
else
